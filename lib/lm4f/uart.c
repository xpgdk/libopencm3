 ...
 *
 * The UART on the LM4F can either be used with a single character TX and RX buffer, or
 * with a 8 character TX and RX FIFO. In order to use the FIFO it must be enabled, this
 * is done with uart_enable_fifo() and can be disabled again with uart_disable_fifo().
 * On reset the FIFO is disabled, and it must be explicitly be enabled.
 * 
 * When enabling the UART FIFOs, RX and TX interrupts are triggered according to the amount
 * of data in the FIFOs. For the RX FIFO the trigger level is defined by how full the FIFO is.
 * The TX FIFO trigger level is defined by how empty the FIFO is instead.
 * 
 * For example, to enable the FIFOs and trigger interrupts for a single received and single
